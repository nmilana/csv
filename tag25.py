...
for fl in lst_d:
    ...
    for item in set_tags:
        ...
        if not i % 25:
            with open(f"out/{_in}/{fl[:-4]}/tag25_{str(int(i / 25)).zfill(2)}.txt", "w", encoding='utf-8') as file:
                file.write(f'{tag25}')
            tag25 = ''
        i += 1
    ...
    if tag25 != '':
        with open(f"out/{_in}/{fl[:-4]}/tag25_{str(int(i / 25) + 1).zfill(2)}.txt", "w", encoding='utf-8') as file:
            file.write(f'{tag25}')   
